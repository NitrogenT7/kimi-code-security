...
记事本（notepad）是你和 Agent 在会话内共享的自由文本缓冲区。Agent 把需要在上下文压缩后保留的工作笔记写在里面——内容存储在对话之外，并会重新附加到压缩摘要中，因此路径、命令、调查发现等精确细节不会在上下文裁剪时丢失。

| 命令 | 作用 | 可用性 |
| --- | --- | --- |
| `/notepad` 或 `/notepad status` | 显示当前记事本内容 | 随时可用 |
| `/notepad edit` | 在外部编辑器中打开记事本，保存后写回 | 随时可用 |

`/notepad edit` 的编辑器解析顺序与外部编辑器快捷键一致：依次为 `/editor` 配置的命令、`$VISUAL`、`$EDITOR`。保存空文件会清空记事本；以非零退出码关闭编辑器则不做任何修改。你的修改在 Agent 下次读取时即可见，因此也可以用记事本给 Agent 留下持久生效的指示。
...
| `/notepad [status\|edit]` | — | 查看或编辑 Agent 记事本，见 [Agent 记事本](#agent-记事本) | 是 |
