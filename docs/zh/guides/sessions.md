...
在 `/sessions` 选择器里，除了浏览和恢复，还可以直接管理会话，无需先切换过去：

- **置顶（`Ctrl+P`）**：把光标所在的会话固定到列表顶部（行首出现 `★` 标记），再按一次取消置顶。置顶状态保存在会话自身的元数据中，关闭终端重新打开后依然生效；最近置顶的会话排在最上面。
- **重命名（`Ctrl+R`）**：对光标所在的会话弹出重命名输入框（已预填当前标题），回车保存、`Esc` 取消。改名后自定义标题不会再被自动生成的标题覆盖。
- **搜索**：选择器打开时直接输入文字即可模糊搜索。搜索范围包括会话标题和上一轮提问的内容——想找「上次聊过登录修复的那个会话」，输入相关关键词就能命中。`Backspace` 删除字符；`Esc` 先清空搜索，再按一次关闭选择器。

::: tip 提示
置顶与重命名对已关闭的会话同样有效（元数据直接落盘），不必先把会话恢复为当前会话。
:::
