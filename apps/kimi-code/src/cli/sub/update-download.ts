...
export async function runUpdateDownloadCommand(
  ...
  if (lock === null) {
    ...
    if (holderVersion === version) {
      ...
      out.write(
        `A download of ${productDisplayName()} ${version} is already in progress; waiting for it to finish…\n`,
      );
      const wait = await waitForStagedUpdate(version, process.execPath, manual);
      if (wait.status === 'staged') {
        out.write(`${productDisplayName()} ${version} is downloaded; it applies on the next start.\n`);
        return 0;
      }
  ...
  const label = `Downloading ${productDisplayName()} ${version} (${process.platform}-${process.arch})…`;
  ...
  try {
    ...
    if (result.status === 'already-staged') {
      out.write(`${productDisplayName()} ${version} is already downloaded; it applies on the next start.\n`);
    }
    return 0;
  } catch (error) {
    if (out.isTTY) out.write('\n');
