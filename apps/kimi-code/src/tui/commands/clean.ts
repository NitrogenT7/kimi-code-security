...
function isCleanable(summary: SessionSummary, cutoffMs: number): boolean {
  if (summary.updatedAt <= 0) return false;
...
export async function handleCleanCommand(host: SlashCommandHost, args: string): Promise<void> {
  ...
  const unknownAge = sessions.filter(
    (s) => s.id !== activeId && s.updatedAt <= 0 && s.isCustomTitle !== true,
  ).length;
  const foundLabel =
    `Found ${String(targets.length)} cleanable of ${String(sessions.length)} session(s) ` +
    `(auto-named, idle ≥ ${String(opts.days)}d, ${scopeLabel})`;
  scan.stop({
    ok: true,
    label: unknownAge > 0 ? `${foundLabel}; ${String(unknownAge)} skipped (unknown age)` : foundLabel,
  });

  if (targets.length === 0) {
    const suffix =
      unknownAge > 0 ? ` ${String(unknownAge)} skipped (unknown age).` : '.';
    host.showStatus(
  ...
  if (!opts.go) {
    const skipNote = unknownAge > 0 ? `\n(${String(unknownAge)} skipped (unknown age))` : '';
    host.showStatus(
