...
describe('TodoPanelComponent (investigation board)', () => {
  it('returns no lines when empty (so the layout slot collapses)', () => {
    const board = new TodoPanelComponent();
  ...
  it('renders the findings area with resolved/inconclusive counts and conclusions', () => {
    const board = new TodoPanelComponent();
    board.setTodos([makeQuestion({ question: 'Active?', status: 'investigating' })]);
    board.setFindings([
  ...
  it('setTodos replaces the list (not appends)', () => {
    const board = new TodoPanelComponent();
    board.setTodos([makeQuestion({ question: 'old' })]);
    board.setTodos([makeQuestion({ question: 'new', status: 'investigating' })]);
  ...
  it('clear() wipes questions and findings and reverts to empty', () => {
    const board = new TodoPanelComponent();
    board.setTodos([makeQuestion({ question: 'x' })]);
  ...
  it('exposes questions and findings for hydration assertions', () => {
    ...
    board.setTodos(questions);
    board.setFindings(findings);
    expect(board.getTodos()).toEqual(questions);
