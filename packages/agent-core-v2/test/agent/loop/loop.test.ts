...
describe('Agent loop', () => {
  ...
  it('fails the turn after a filtered step completes', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [wire] llm.request                 { "agentId": "main", "kind": "loop", "provider": "openai", "model": "mock-model", "modelAlias": "mock-model", "thinkingEffort": "off", "maxTokens": 1000000, "toolSelect": false, "systemPromptHash": "ec9c34379c88babbc468ef2f3e0e08cd2f422c8c4a910664fb8bb394d703a575", "toolsHash": "8ec55b0a470cebfc73aeca5ceb8e709e55e6a714f69d741a84a75e784c63aa4a", "messageCount": 1, "turnStep": "0.1", "time": "<time>" }
...
describe('mid-turn model switch', () => {
  it('applies a model switch between steps of the same turn', async () => {
    ...
    try {
      vi.stubEnv('KIMI_CODE_EXPERIMENTAL_MID_TURN_MODEL_SWITCH', '1');
      let switched = false;
