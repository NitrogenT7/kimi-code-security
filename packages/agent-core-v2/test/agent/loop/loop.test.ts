...
describe('mid-turn model switch', () => {
  it('applies a model switch between steps of the same turn', async () => {
    // Two configured models; the loop starts on mock-model and the tool
    // execution mid-turn switches the profile to alt-model — mirroring what
    // profileService.setModel does on a mid-stream /model selection.
    let callCount = 0;
    ...
    try {
      ...
      await ctx.untilTurnEnd();
      // The wire record for the second request is written during the turn;
      // allow a tick for the harness snapshot to settle.
      await new Promise((resolve) => setTimeout(resolve, 50));

      // First llm.request used the initial model; after the mid-turn
      // setModel the turn's model snapshot is invalidated, so the next
      // request in the same turn resolves to alt-model.
...
describe('aborted step tool execution', () => {  it('accounts model usage when the step is aborted during tool execution', async () => {
    const ctx = createTestAgent(
