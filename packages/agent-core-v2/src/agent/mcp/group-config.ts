/**
 * `mcp` domain (L5) — MCP server group declarations (`mcpGroups` in mcp.json).
 *
 * Groups are named bundles of MCP servers with an optional description and an
 * optional `skillPrefixes` sandbox list. They are loaded from the same three
 * locations as MCP servers (`~/.kimi-code/mcp.json`, `<project root>/.mcp.json`,
 * `<cwd>/.kimi-code/mcp.json`); later files override earlier ones by group
 * name. When no groups are declared but servers exist, default security-
 * research groups are auto-generated (matching the legacy Python kimi-cli).
 */
...
export function generateDefaultMcpGroups(serverNames: readonly string[]): Record<string, McpGroup> {
  ...
  > = {
    ...
    binary: {
      ...
      // Allow binary-specific skills, generic audit/review skills, and code
      // review skills.
