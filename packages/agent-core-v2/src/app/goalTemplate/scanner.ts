/**
 * `goalTemplate` domain (L3) — goal template discovery.
 *
 * Scans the project `.goal/` directory and the user goal directories
 * (`~/.goal/`, `~/.agents/goals/`) for `.md` templates, parsing each through
 * `parseGoalTemplateFromFile`. Duplicate template names keep the first
 * discovery (project roots scan before user roots); malformed templates are
 * reported through `onWarning` and skipped. The fs probes are injectable so
 * tests can run without touching the disk.
 */
...
import { SkillParseError } from '#/app/skillCatalog/parser';

import { parseGoalTemplateText } from './parser';
...
export interface ParseGoalTemplateFromFileOptions {
...
export async function parseGoalTemplateFromFile(
  options: ParseGoalTemplateFromFileOptions,
