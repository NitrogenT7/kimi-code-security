...
function registerSecurityRole(spec: SecurityRoleSpec): void {
  ...
  registerAgentProfile({
    ...
    renderSystemPrompt: (context) =>
      renderSystemPromptResult(role, context, { skillActive: true }),
  });
}
...
registerSecurityRole({
  ...
  whenToUse:
    'Use this agent for security-research tasks that do not fit a specialized role, or that span several MCP groups. The main agent must load the required MCP group (/mcp:<group> or MCPManager load_group) before delegating.',
  role:
