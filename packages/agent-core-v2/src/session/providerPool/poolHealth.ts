/**
 * `providerPool` domain (L6) — endpoint health tracking, cooldown backoff,
 * and the hourly recovery prober (pure state machines, no DI).
 *
 ...
 * Request-time failover lives in `poolingModel.ts`; resolution of pool
 * endpoints and prober lifecycle live in `providerPoolService.ts`.
 *
 * Port of v1 `agent-core/session/provider-pool.ts`; error classifiers come
 * from the v2 `llmProtocol` fork.
 */

import { APIStatusError, isProviderRateLimitError } from '#/app/llmProtocol/errors';

import type { PoolConfig } from './configSection';
