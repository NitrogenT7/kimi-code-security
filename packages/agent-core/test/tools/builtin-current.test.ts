...
describe('current builtin collaboration tools', () => {
  ...
  it('AgentSwarm forwards per-item models and renders model/usage attributes', async () => {
    const host = mockSubagentHost({
      runQueued: vi.fn().mockResolvedValue([
        {
          task: {
            kind: 'spawn',
            data: { kind: 'spawn', index: 1, item: 'a', prompt: 'Review a', model: 'ds-flash' },
            profileName: 'coder',
            parentToolCallId: 'call_swarm',
            prompt: 'Review a',
        ...
        },
        {
          task: {
            kind: 'spawn',
            data: { kind: 'spawn', index: 2, item: 'b', prompt: 'Review b' },
            profileName: 'coder',
            parentToolCallId: 'call_swarm',
            prompt: 'Review b',
    ...
    expect(AgentSwarmToolInputSchema.safeParse(input).success).toBe(true);
    expect(AgentSwarmToolInputSchema.safeParse({ ...input, items: [{ item: '' }] }).success).toBe(
      false,
    );
